...
RSpec.describe Currency, type: :model do
  ...
  it 'returns correct json object' do
    ...
    create(:exchange_rate, base_currency_id: base_currency.id, converted_currency_id: converted_currency_id.id, rate: 0.8)
    expect(base_currency.to_json[:rates][0].keys).to contain_exactly(:country, :rate)
    expect(base_currency.to_json[:rates][0].values).to contain_exactly("GBP", 0.8)
  end
end
